...
text_clean = "".join(
    list(map(lambda c: c if c >= 'а' and c <= 'я' or c >= 'a' and c <= 'z' or c == " " or c == "-" else "", text)))
...
words_count = reduce(lambda y, x: y.update([(x, y.get(x, 0) + 1)]) or y, words, {})
print(words_count)
...
for i in range(len(words_count)):
    ll = [k for k, v in words_count.items() if v == i]
    if ll:
        words_dict[i] = ll[:3]
print(words_dict)
